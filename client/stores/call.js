...
export const useCallStore = defineStore('call', {
  ...
  actions: {
    ...
    setContactListShow() { // payload 제거, isContactListShow를 토글하는 것으로 변경
    ...
    pushPdfUploadQueArray(payload) {
      const queArray = { // new Object() 대신 객체 리터럴 사용
    ...
    removePdfUploadQueArray() { // payload 제거
    ...
    setLaserPointerLaction(payload) {
      ...
      const locationArray = { // new Object() 대신 객체 리터럴 사용
        x: payload.x,
        y: payload.y,
      };
    ...
    setCaptureImageInfo(payload) {
      this.captureImageInfo = { // new Object() 대신 객체 리터럴 사용
    ...
    addMotionFallInfo(payload) {
      const motionArray = { // new Object() 대신 객체 리터럴 사용
    ...
    clearMotionFallInfo() { // payload 제거
      this.motionFallInfo = [];
    },
    ...
    setMyGpsListPush(payload) {
      if (this.myGpsList) { // myGpsList가 null이 아닌 경우에만 push
    ...
    toggleFlag() { // payload 제거, 토글 로직
      console.log(this.flag);
      this.flag = !this.flag;
      console.log(this.flag);
    },
    setChangeZoomLevel(payload) { // 이름 변경: changeZoomLevel -> setChangeZoomLevel (mutation 이름과 통일)
      console.log(payload);
    ...
    toggleDeclineStatus() { // payload 제거, 토글 로직
