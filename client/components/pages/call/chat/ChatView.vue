<template>
    <div class="row content-start messageBox">
      ...
      <div class="row " v-else>
        ...
        <div class="col">
          <span class="messageBoxDate" :class="[compData.isReceived ? 'receptionTime' : 'outgoingTime']">{{ getTime(compData.chattingDate) }}</span>
        </div>
        <div class="col-auto">
          <span v-if="!compData.isReceived" class="caller messageBoxNickname">{{ compData.nickname }}</span>
        </div>
      </div>
      ...
      >
        ...
        <span class="text-center arrowIconMoveText" :style="{lineHeight: locale == 'ko' ? '50px' : '50px' }">{{ moveIcons[compData.moveIcon-1].text }}</span>
      </div>
      <div
