<template>
    <div class="column content-start sendMessageBox">
      <div class="col-auto row justify-between line1">
        <div class="col row">
          ...
          <button
            ...
            :style="{backgroundColor: messageType == 2 ? '#EE314A' : displayType == 'lightmode' ? '#fff' : '#4D4D4D', color: messageType == 2 && (displayType == 'darkmode' || displayType == 'lightmode') ? '#fff' : displayType == 'darkmode' ? '#fff' : 'black'}"
            v-if="!videoCallHost"
            class="emergency"
          >
            {{ t("emergency") }}
          ...
          <button
            ...
            :style="{backgroundColor: messageType == 4 && displayType == 'darkmode' ? '#7E7E7E' : messageType == 4 && displayType == 'lightmode' ? '#D6D6D6' : displayType == 'darkmode' ? '#4D4D4D' : '#fff'}"
          ...
          <button
            ...
            :style="{backgroundColor: messageType == 5 && displayType == 'darkmode' ? '#7E7E7E' : messageType == 5 && displayType == 'lightmode' ? '#D6D6D6' : displayType == 'darkmode' ? '#4D4D4D' : '#fff'}"
            v-if="videoCallHost && !isDrawing"
            class="emergency"
          >
        ...
        <div class="col-auto row justify-end iconContainer">
          <button
            ...
            :style="{backgroundColor: messageType == 3 ? '#EE314A' : displayType == 'lightmode' ? '#fff' : '#4D4D4D'}"
            v-if="videoCallHost && !isDrawing"
            class="row justify-center items-center icon"
          >
            <img v-if="displayType == 'lightmode' && messageType != 3" src="@/assets/images/lightmode/chattingbar/ic_alarm.svg">
      ...
      <div class="col row textareaContainer" v-if="messageType < 3">
        <button class="col-auto" id="focusTextAreaBack" v-if="view" style="background: #4a4a4a; width: 30px; font-size: 40px;" @click="focusTextAreaBack">←</button>
        <textarea v-model="sendMessageText" @keyup.enter="makeEnterEvent" @focus="sendMessageTextfocus" class="col-12" id="sendMessageText"></textarea>
        <button @click="sendMessage" :style="{backgroundColor: messageType == 2 ? '#EE314A' : '#1c8eff'}" class="col-12 sendMessageBtn">
          {{ t("send message") }}
        </button>
      </div>
      <div class="col row" v-else-if="messageType == 3">
        <div class="col-4 emergencyIconsContainer" v-for="(emergencyIcon, emergencyIconKey) in emergencyIconList" :key="emergencyIconKey">
          <button @click="sendMessage(emergencyIconKey)" :style="{backgroundColor: emergencyIconKey < 3 ? '#EB3149' : '#FF990F' }" class="column justify-center items-center emergencyIcon">
            <img :src="emergencyIcon.img">
            <span>{{ emergencyIcon.text }}</span>
          </button>
        </div>
      </div>
      <div class="col row" v-else-if="messageType == 4">
        <div class="col arrowDiv col-4 arrowIconsContainer" v-for="(arrowIcon, arrowIconKey) in moveIconList" :key="arrowIconKey">
          <button @click="sendMessage(arrowIconKey)" class="column justify-center items-center arrowIcon">
            <img :src="arrowIcon.img">
            <span>{{ arrowIcon.text }}</span>
          </button>
        </div>
      </div>
      <div class="col row" v-else-if="messageType == 5">
        <div class="col arrowDiv col-4 arrowIconsContainer" v-for="(arrowIcon, arrowIconKey) in directionIconList" :key="arrowIconKey">
          <button @click="sendMessage(arrowIconKey)" class="column justify-center items-center arrowIcon">
            <img :src="arrowIcon.img">
            <span>{{ arrowIcon.text }}</span>
          </button>
...
const emergencyIconList = computed(() => [
  { img: new URL('@/assets/images/messageBox/ic_fire.png', import.meta.url).href, text: t('fire') },
  { img: new URL('@/assets/images/messageBox/ic_emergency.png', import.meta.url).href, text: t('emergencies') },
  { img: new URL('@/assets/images/messageBox/ic_ax.png', import.meta.url).href, text: t('accident') },
  { img: new URL('@/assets/images/messageBox/ic_car.png', import.meta.url).href, text: t('patient transportation') },
  { img: new URL('@/assets/images/messageBox/ic_hand.png', import.meta.url).href, text: t('work order') },
  { img: new URL('@/assets/images/messageBox/ic_check.png', import.meta.url).href, text: t('on-site confirmation') }
]);

const moveIconList = computed(() => [
  { img: new URL('@/assets/images/messageBox/ic_move_up.png', import.meta.url).href, text: t('chatting up') },
  { img: new URL('@/assets/images/messageBox/ic_move_left.png', import.meta.url).href, text: t('chatting left') },
  { img: new URL('@/assets/images/messageBox/ic_move_front.png', import.meta.url).href, text: t('chatting front') },
  { img: new URL('@/assets/images/messageBox/ic_move_down.png', import.meta.url).href, text: t('chatting down') },
  { img: new URL('@/assets/images/messageBox/ic_move_right.png', import.meta.url).href, text: t('chatting right') },
  { img: new URL('@/assets/images/messageBox/ic_move_back.png', import.meta.url).href, text: t('chatting back') }
]);

const directionIconList = computed(() => [
  { img: new URL('@/assets/images/messageBox/ic_direction_up.png', import.meta.url).href, text: t('chatting up') },
  { img: new URL('@/assets/images/messageBox/ic_direction_left.png', import.meta.url).href, text: t('chatting left') },
  { img: new URL('@/assets/images/messageBox/ic_direction_front.png', import.meta.url).href, text: t('chatting front') },
  { img: new URL('@/assets/images/messageBox/ic_direction_down.png', import.meta.url).href, text: t('chatting down') },
  { img: new URL('@/assets/images/messageBox/ic_direction_right.png', import.meta.url).href, text: t('chatting right') },
  { img: new URL('@/assets/images/messageBox/ic_direction_back.png', import.meta.url).href, text: t('chatting back') }
]);
...
const sendMessage = (iconKey) => {
  ...
  if (messageType.value === 2) { // emergency
  ...
  } else if (messageType.value === 3) { // icon Message : emergencyIcons
    chattingStore.sendMessage({
      nickname: sessionStorage.getItem("m_nickname"),
      date: nowDate,
      chattingDate: getChattingTimeZone(nowDate),
  ...
  } else if (messageType.value === 4) { // icon Message : Move Icon
    chattingStore.sendMessage({
      nickname: sessionStorage.getItem("m_nickname"),
      date: nowDate,
      chattingDate: getChattingTimeZone(nowDate),
  ...
  } else if (messageType.value === 5) { // icon Message : Direction Icon
    chattingStore.sendMessage({
      nickname: sessionStorage.getItem("m_nickname"),
      date: nowDate,
      chattingDate: getChattingTimeZone(nowDate),
  ...
  setTimeout(() => {
    ...
    if (scrollElement) { // Add null check for scrollElement
...
const sendMessageTextfocus = (event) => {
  ...
  if (window.innerWidth < 1023 && accessDeviceCheck.value === "Mobile") {
    ...
    if (sendMessageBtn) { // Add null check
...
const focusTextAreaBack = () => {
  if (window.innerWidth < 1023 && accessDeviceCheck.value === "Mobile") {
    ...
    if (chatFocusEle && chatFocusEle.parentElement) { // Add null check for parentElement
...
.textareaContainer {
  ...
  > textarea {
    padding: 8px;
    height: 144px; // Explicit height for textarea, ensure it plays well with mobile focus logic
    margin-bottom: 3px;
  }
}
...
.emergencyIconsContainer {
  ...
  &:nth-child(5) > .emergencyIcon {
    padding-top: 10px; // Specific override
  }
}
